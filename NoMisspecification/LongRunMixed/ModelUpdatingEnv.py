...
class ModelUpdatingEnv(gym.Env):
    ...
    def __init__(self, initial_X, initial_Y, initial_coefficients, initial_probs, update_penalty, n_features, n_timesteps):
        ...
        self.lower_thresholds = np.repeat(-1, n_features)
        # define upper thresholds
        self.upper_thresholds = np.repeat(1, n_features)
    ...
    def step(self, action):
        ...
        if np.random.rand() < 0.01:
            ...
            self.real_coefficients = np.random.uniform(low=self.lower_thresholds, high=self.upper_thresholds, size=self.n_features)
        else:
